which.var <- function(variable, where.to){
    if(is.numeric(variable)){
        n <- where.to[variable]
        i <- variable
    } else if (is.character(variable)){
        n <- variable
        i <- match(variable, where.to)
    }
    return(list(n=n, i=i))
}

cup.boxplot <- function(data, var){
    s.var <- which.var(var, names(data))
    print(s.var$i)
    base.graph <- ggplot(data = data, aes_string(x = 'identity', y = s.var$n))
    base.graph + 
        geom_boxplot(fill = '#3399CC', colour = 'black', 
                     outlier.colour = 'red', outlier.size = 3) +
        ggtitle(paste('Box-plot ', s.var$n)) + 
        scale_y_continuous(name = '') + 
        scale_x_continuous(name = '', breaks = NULL) +
        theme(plot.title = element_text(lineheight = .8, face = 'bold'))
}
cup.boxplot(diamonds, "x")
